    ...
    for i in range(6, n - 3):
        ...
        if is_push2_long and push2_score_l >= 3:
            ...
            for gap in range(1, 4):
                ...
                # V17A: Weighted Book Pressure filter for LONG
                # Block if book_pressure > 0.9 (no seller exhaustion near close)
                bp_l = c.get("book_pressure_ratio", 1.0)
                if bp_l > 0.9:
                    continue
        ...
        if is_push2_short and push2_score_s >= 3:
            ...
            for gap in range(1, 4):
                ...
                # V17A: Weighted Book Pressure filter for SHORT
                # Block if book_pressure < 1.2 (no buyer exhaustion near close)
                bp_s = c.get("book_pressure_ratio", 1.0)
                if bp_s < 1.2:
                    continue
                        ...
                        if total_score >= 7:
                            ...
                            if R > atr * 0.15 and R <= atr * max_stop:
                                ...
                                if current_vwap > entry:
                                    ...
                                    # V17A: Weighted Book Pressure filter for SHORT cascade
                                    bp_sc = c.get("book_pressure_ratio", 1.0)
                                    if bp_sc < 1.2:
                                        continue
