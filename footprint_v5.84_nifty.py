...
class FootprintRenderer:
    ...
    def render_candle(self, candle, prev_candle=None):
        ...
        # GoCharting "default" colorMatrix — reverse-engineered from app.js Da() function.
        # bid_ask column: intensity = |buy-sell| / denom, then linear interpolation between two hex colors.
        # GREEN (buy>sell): #8faf8e (143,175,142) → #12aa0d (18,170,13); denom = maxSell
        # RED (sell>buy):   #d37c85 (211,124,133) → #ef1329 (239,19,41); denom = maxBuy
        # GoCharting initializes minBuy/minSell to 0, so |minBuy-maxSell| = maxSell, |maxBuy-minSell| = maxBuy
        # Luminance: green = 161.7 - 55.0*i; red = 151.1 - 63.7*i
        ...
        for i, row in enumerate(complete_rows):
            ...
            if row['buy'] > row['sell']:
                ...
                denom = red_denom
            intensity = min(abs_delta / denom, 1.0) if denom > 0 else 0
                ...
                if has_pattern:
                    ...
                    if atr and atr > 0 and 'DG2' in matched_prices:
                        ...
                        if effective_exhaust:
                            ...
                            # 🔥 V5.85: VWAP-ABSORB floor — compressed ATR during consolidation shouldn't
                            # block patterns at MIN_PATTERN_HEIGHT when VWAP support + strong absorption confirm.
                            # Uses direct VWAP proximity (D2 above VWAP) + absorb≥5 to bypass ATR compression.
                            _d2_above_vwap = (self.session_vwap is not None and 'DG2' in matched_prices
                                              and matched_prices['DG2'] >= self.session_vwap)
                            if _d2_above_vwap and absorb_score >= 5 and max_height < MIN_PATTERN_HEIGHT_PTS:
                                max_height = MIN_PATTERN_HEIGHT_PTS
                                _vwap_absorb_floor = True
                        ...
                        if pattern_height < min_height:
                            ...
                            vwap_floor_tag = " [VWAP-ABSORB-FLOOR]" if _vwap_absorb_floor else ""
                            debug_trap_logs.append(f"[HEIGHT] OK grade {grade_label} ({height_for_filter:.1f} in [{min_height:.1f}, {max_height:.1f}]){ec_tag}{vwap_floor_tag}{inflate_tag}")
                    ...
                    if signal_blocked_reason is not None:
                        print(f" \033[93m⚠ LONG BLOCKED [{signal_blocked_reason}]\033[0m"
                              f" Trap:{target_trap:.2f} | Entry~{ohlc_close:.2f}"
                              f" | Stop~{target_trap - (atr or 0)*0.1:.2f}"
                              f" | R:{ohlc_close - target_trap:.1f}pts")
        ...
        if self.bear_active_ceiling_price is not None:
            ...
            if new_ceil_idx >= 0:
                ...
                if bear_has_pattern:
                    ...
                    if bear_blocked_reason is not None:
                        print(f" \033[93m⚠ SHORT BLOCKED [{bear_blocked_reason}]\033[0m"
                              f" Ceil:{target_ceiling:.2f} | Entry~{ohlc_close:.2f}"
                              f" | Stop~{target_ceiling + (atr or 0)*0.1:.2f}"
                              f" | R:{target_ceiling - ohlc_close:.1f}pts")
