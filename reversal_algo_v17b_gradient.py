    ...
    for i in range(6, n - 3):
        ...
        if is_push2_long and push2_score_l >= 3:
            ...
            for gap in range(1, 4):
                ...
                # V17B: Book Gradient filter for LONG
                # Block if bid_gradient - ask_gradient >= 0.5 (buyers front-loaded = exhaustion)
                bg_l = c.get("bid_gradient", 0) - c.get("ask_gradient", 0)
                if bg_l >= 0.5:
                    continue
        ...
        if is_push2_short and push2_score_s >= 3:
            ...
            for gap in range(1, 4):
                ...
                # V17B: Book Gradient filter for SHORT
                # Block if ask_gradient - bid_gradient >= 0.5 (sellers front-loaded = selling exhausted)
                bg_s = c.get("ask_gradient", 0) - c.get("bid_gradient", 0)
                if bg_s >= 0.5:
                    continue
                        ...
                        if total_score >= 7:
                            ...
                            if R > atr * 0.15 and R <= atr * max_stop:
                                ...
                                if current_vwap > entry:
                                    ...
                                    # V17B: Book Gradient filter for SHORT cascade
                                    bg_sc = c.get("ask_gradient", 0) - c.get("bid_gradient", 0)
                                    if bg_sc >= 0.5:
                                        continue
